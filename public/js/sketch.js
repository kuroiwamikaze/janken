...
function draw() {
  ...
  if (gestures_results) {
    ...
    if (gestures_results.gestures.length > 0) {
      ...
      if (name == 'one-samples' && mode == 1) {
        mode = 2;
        socket.emit('ready', '');
      }
      else if (name != 'one-samples' && mode == 2) {
        mode = 1;
        socket.emit('unready', '');
      }
  ...
  text(score_message, width / 2, height / 2 + 48);
  text(zannen_message, width / 2, height / 2 + 48);
  textSize(15);
  text(rast_message, width / 2, height / 2 + 96);
